...
import { convertFromJpg } from "../actions";
import { toast } from "sonner";
import { ConversionOptions } from "./ConversionOptions";
import { ConvertedImagePreview } from "./ConvertedImagePreview";

export function ConvertFromJpgForm() {
  ...
  const handleConvert = async (formData: FormData) => {
    if (files.length === 0) {
      toast.error("Please upload at least one JPG image");
      return;
    }
    ...
    try {
      const results = await Promise.all(
        files.map(async (file) => {
          ...
          return convertFromJpg(fileFormData);
        })
      );
